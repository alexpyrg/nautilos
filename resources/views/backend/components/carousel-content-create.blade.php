...
{{--@section('content')--}}
{{--    <div>--}}
{{--        <h1> Δημιουργία Carousel | {{ $page->name }}</h1>--}}
...
@section('content')
    <div class="p-8 bg-gray-100 min-h-screen">
        <div class="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-lg">
            <h1 class="text-2xl font-semibold text-gray-800 mb-6">Δημιουργία Carousel | {{ $page->name }}</h1>
            <form method="post" action="/admin/carousel/image/save" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <input type="hidden" name="page_id" value="{{ $page->id }}">
                <input type="hidden" name="carousel_id" value="{{ $carousel->id }}">
                ...
                <!-- Submit Button -->
                <div>
                    <button type="submit" class="w-full bg-blue-600 text-white font-medium py-2 rounded-lg shadow hover:bg-blue-700 focus:ring focus:ring-blue-500">
                        Αποθήκευση
                    </button>
                </div>
            </form>
