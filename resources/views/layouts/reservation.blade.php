...
{{--    @vite(['resources/css/wickedpicker.scss']);--}}
    ...
    <style>
        @if(App::getLocale() == 'gr' || App::getLocale() == 'GR')
           .form-container *:not(.material-symbols-outlined){
            font-family: 'sans-serif' !important;
        }
        .container > *:not(.material-symbols-outlined){
            font-family: 'sans-serif' !important;
        }
        @endif
    </style>
...
<script>
    document.getElementById('timeSelect').addEventListener('focus', () => {
        const selectedOption = document.querySelector('#timeSelect option[value="20:00"]');
        const selectElement = document.getElementById('timeSelect');
        selectElement.scrollTop = selectedOption.offsetTop;
    });
</script>
